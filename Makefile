...
.PHONY: help up up-dev up-prod down restart build rebuild ps logs \
        logs-arango logs-surreal logs-lm logs-bridge shell-bridge shell-arango \
        health clean init
...
logs-surreal:                      ## Tail surrealdb logs
	$(COMPOSE) logs -f --tail=50 surrealdb
...
health:                            ## Smoke-check that services are reachable
	...
	@echo "--- SurrealDB:" && \
		curl -fsS -u root:$${SURREAL_ROOT_PASSWORD:-password} \
			http://localhost:8000/health >/dev/null && echo "  UP" || echo "  DOWN"
	@echo "--- LM Studio (host):" && \
