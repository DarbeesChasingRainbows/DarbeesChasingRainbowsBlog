...
/// </summary>
public sealed class ArangoRecallEngine : IRecallEngine
{
    ...
    public async Task<IReadOnlyList<GraphCandidate>> GraphExpandAsync(
        TenantId tenantId, IReadOnlyList<string> entityIds, int expandHops, CancellationToken ct = default)
    {
        ...
        var rows = await _repository.QueryAsync<GraphRow>(aql, bindVars, ct);
        ...
        string? createdAt, string? updatedAt, float[]? embedding)
    {
        ...
        return new MemoryItem(
            ...
            Text: text ?? "",
            Embedding: embedding,
            TenantId: new TenantId(tenantId ?? "public"),
            Status: status ?? "ready",
    ...
    public async Task<IReadOnlyList<VectorCandidate>> VectorTopKAsync(
        TenantId tenantId, float[] queryEmbedding, int limit, CancellationToken ct = default)
    {
        var contentCols = new[]
        {
        ...
        foreach (var collection in contentCols)
        {
            ...
            var rows = await _repository.QueryAsync<VectorRow>(aql, bindVars, ct);
