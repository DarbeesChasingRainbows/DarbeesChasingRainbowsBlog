...
public sealed class ArangoMemoryRepository : IMemoryRepository, ISchemaManager, IEmbeddingMigrator, IDisposable
{
    ...
    public ArangoMemoryRepository(string url, string db, string user, string pass, string embeddingModelId, int embeddingDimension, int vectorNLists, HttpClient rawHttp, IDomainEventDispatcher dispatcher, IEmbeddingClient? embeddings = null)
    {
        ...
        CancellationToken ct = default)
    {
        ...
        await _dispatcher.DispatchAsync(new EmbeddingConfigChangedEvent(previous, current, queueSizeAfter, DateTime.UtcNow), ct);
        ...
        CancellationToken ct = default)
    {
        ...
        if (summaryOutcome == VectorWriteOutcome.Embedded || bodyOutcome == VectorWriteOutcome.Embedded)
        {
    ...
    public async Task<UpsertNoteResult> UpsertNoteAsync(NoteDocument note, CancellationToken ct = default)
    {
        ...
        var arangoKey = ContentHash.Sha1Hex(note.Key);
        var hash = ContentHash.Sha256Hex($"{_embeddingModelId}\n{note.Text}");
    ...
    {
        var arangoKey = ContentHash.Sha1Hex(noteKey);
        ...
        CancellationToken ct = default)
    {
        ...
        if (kinds.Contains(MemoryKind.Post))
        {
            ...
            var bindVars = new Dictionary<string, object>
            {
                ["@col"] = MemoryCollections.Posts,
                ["query_vec"] = queryVec,
                ["tenants"] = tenants.Select(t => t.Value).ToArray(),
                ["raw_k"] = rawK,
            };
            ...
            allHits.AddRange(cursor.Result.Select(r => new PostSearchHit
            {
                ...
                Kind = r.kind ?? "post",
                TenantId = r.tenant_id != null ? new TenantId(r.tenant_id) : null,
                Title = r.title ?? "",
                Text = r.text ?? "",
