 ...
 */
async function assembleBlocks(bodyBlocks, richMedia, importPrefix, collection, slug, exportPath, services, schema) {
  ...
  for (const block of blocks) {
    switch (block.type) {
      ...
      case 'header': {
        const prefix = '#'.repeat(block.data?.level || 2);
        mdxParts.push(`${prefix} ${block.data?.text || ''}`);
        break;
      }
...
async function resolveImage(fileId, collection, slug, exportPath, services, schema) {
  ...
  try {
    ...
    const ext = file.filename_download?.split('.').pop() || 'jpg';
 ...
 */
export async function exportItem(item, collection, exportPath, services, schema, logger) {
  ...
  const body = await assembleBlocks(item.body_blocks, item.rich_media, config.importPrefix, collection, slug, exportPath, services, schema);
...
export { assembleBlocks, processRichBlock };
